...
Syscall_Spawn(uint64_t user_path, uint64_t user_argv)
{
	...
	*str = (uintptr_t)argstart;
    ...
    char* outarg = (char*)argstart;
    uintptr_t* outarr = (uintptr_t*)argstart;
    uintptr_t* inarr = (uintptr_t*)arg;
    memset(outarr, 0, sizeof(uintptr_t) * 8);
    ...
    for (int i = 1; i < 8; i++) {
        ...
        char buffer[256];
        strcpy(buffer, (char*)inarr[i]);

        int len = strlen(buffer) + 1;
        buffer[len - 1] = '\0';

        memcpy(offset + outarg, buffer, len);
        outarr[i] = offset + (MEM_USERSPACE_STKTOP - PGSIZE);
        offset = offset + len;
    }

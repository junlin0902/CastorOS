...
O2FSGrowVNode(VNode *vn, uint64_t filesz)
{
    ...
    if (filesz > (vfs->blksize * O2FS_DIRECT_PTR * O2FS_DIRECT_PTR)) {
        return -EINVAL;
    }

    for (int i = blkstart; i < ((filesz + vfs->blksize - 1) / vfs->blksize); i++) {
        uint64_t indirectIndex = i / O2FS_DIRECT_PTR;
        uint64_t directIndex = i % O2FS_DIRECT_PTR;

        uint64_t blkno = O2FSBAlloc(vfs);
        if (bn->indirect[indirectIndex].offset == 0) { 
            if (blkno == 0) {
                return -ENOSPC;
            }
            bn->indirect[indirectIndex].offset = blkno * vfs->blksize;
            BufCache_Read(vfs->disk, bn->indirect[indirectIndex].offset, &ient);
        ...
        if (bind->direct[directIndex].offset != 0) {
            BufCache_Release(ient);
            continue;
        }

        
        uint64_t directBlkno = O2FSBAlloc(vfs);
        if (directBlkno == 0) {
            BufCache_Release(ient);
            return -ENOSPC;
        }
        ...
        bind->direct[directIndex].offset = directBlkno * vfs->blksize;
...
O2FSResolveBuf(VNode *vn, uint64_t b, BufCacheEntry **dentp)
{
    ...
    uint64_t indirectIndex = b / O2FS_DIRECT_PTR;
    uint64_t directIndex = b % O2FS_DIRECT_PTR;

    status = BufCache_Read(vn->disk, bn->indirect[indirectIndex].offset, &ient);
    if (status < 0) {
        return status;
    }
    bind = (BInd *)ient->buffer;

    status = BufCache_Read(vn->disk, bind->direct[directIndex].offset, &dent);
    if (status < 0) {
        return status;
    }

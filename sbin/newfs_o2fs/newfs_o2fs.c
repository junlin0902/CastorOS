...
ObjID *AddFile(const char *file)
{
    ...
    uint64_t bind_offset = AppendEmpty();
    node.indirect[j].device = 0;
    node.indirect[j].offset = bind_offset;
	...
	bind.direct[i].offset = offset;
	node.size += (uint64_t)len;
    FlushBlock(bind_offset, &bind, sizeof(bind));
    i += 1;
...
ObjID *AddDirectory()
{
    ...
    uint64_t bind_offset = AppendBlock(&bind, sizeof(bind));
    node.indirect[0].device = 0;
    node.indirect[0].offset = bind_offset;
